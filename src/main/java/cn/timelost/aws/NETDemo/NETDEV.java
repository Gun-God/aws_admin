...
import cn.timelost.aws.NETDemo.Struct.NETDEV_PREVIEWINFO_S;
...
//            JOptionPane.showMessageDialog(this, "当前用户未登录");
        ...
        if (Pointer.NULL != NetDEVSdk.m_lpPlayHandle) {
            iRet = ITF.NETDEV_StopRealPlay(NetDEVSdk.m_lpPlayHandle);
        ...
        if (Pointer.NULL != NetDEVSdk.m_lpPlayHandle) {
            iRet = ITF.NETDEV_StopPlayBack(NetDEVSdk.m_lpPlayHandle);
            if (NetDEVEnum.TRUE == iRet) {
                NetDEVSdk.m_lpPlayHandle = Pointer.NULL;
            }
        }
        ...
        String errMessage;
        NETDEV_PREVIEWINFO_S.ByReference pstPreviewInfo = new NETDEV_PREVIEWINFO_S.ByReference();

        pstPreviewInfo.dwChannelID = m_lChannelID;
        pstPreviewInfo.hPlayWnd = NetDEVSdk.m_lpPlayWndHandle;
...
//            NetDEVSdk.m_lpPlayHandle.getLong(0);
    ...
    private void RealStopActionPerformed() {//GEN-FIRST:event_btnRealStopActionPerformed
        ...
        if (Pointer.NULL != NetDEVSdk.m_lpPlayHandle) {
            int iRet = ITF.NETDEV_StopRealPlay(NetDEVSdk.m_lpPlayHandle);
            if (NetDEVEnum.TRUE != iRet) {
                //  JOptionPane.showMessageDialog(this, "实况流停流失败.");
                return;
            } else {
                //  JOptionPane.showMessageDialog(this, "实况流停流成功.");
            }
        }
        NetDEVSdk.m_lpPlayHandle = Pointer.NULL;
